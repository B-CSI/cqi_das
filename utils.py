...
def filter_imported_data(data: pd.DataFrame,pass_low=3, pass_hi=18, freq=50, decimate_data=False):
    ...
    if decimate_data == True:
        df_fil = data.apply(lambda x: (obspy.signal.filter.bandpass(np.array(x), freqmin=pass_low, freqmax=12.5, df=freq))) 
        if freq != 50:
            df_fil = df_fil.apply(lambda x: (decimate(x, int(freq/50)))) #changing sample frequency for all to 50 Hz
    else:
        df_fil = data.apply(lambda x: (obspy.signal.filter.bandpass(np.array(x), freqmin=pass_low, freqmax=pass_hi, df=freq))) 
...
def load_bad_channels(filename: str, first_ch: int, candas1=False):
    if candas1 is True:
        ...
        return bad_channels_candas1_2nd_picked[np.where(bad_channels_candas1_2nd_picked.T[1] == 1)].T[0]
